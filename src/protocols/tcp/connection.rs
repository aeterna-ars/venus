...
pub struct TcpConnection {
    pub config: Arc<TcpCommonConfig>,
    pub fd: i32,
...
impl TcpConnection {
    ...
    pub async fn read_frame(&mut self) -> Result<BytesMut, TcpError> {
        ...
        let n: usize = timeout(duration, self.read_in.read_buf(&mut self.read_buf))
            .await
            .map_err(|_| TcpError::Timeout("idle timeout"))?
    ...
    async fn send(&mut self) -> Result<(), TcpError> {
        if self.config.no_delay {
            let max: usize = self.config.max_fragment_size.unwrap() as usize;
            let mut interval = interval(self.config.sending_interval_nanosecs.unwrap());
...
fn n_check(n: usize) -> Result<(), TcpError> {
    if n == 0 {
        return Err(TcpError::Timeout("no received bytes"))
    }
